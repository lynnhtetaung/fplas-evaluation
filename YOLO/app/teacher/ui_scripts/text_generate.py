...
def generate_text_widget():
    text = random.choice(["Welcome", "Hello World", "Choose an option", "Flutter Demo", "Sample Text"])
    font_size = random.choice([20, 24, 26, 28, 30])
    color = random.choice(["Colors.black", "Colors.blue", "Colors.green", "Colors.red", "Colors.purple"])
...
for i in range(1, num_files + 1):
    dart_code = generate_text_widget()
    dart_file_path = os.path.join(dart_files_output_dir, f"text_{i}.dart")
    screenshot_path = os.path.join(screenshot_output_dir, f"text_{i}.png")
